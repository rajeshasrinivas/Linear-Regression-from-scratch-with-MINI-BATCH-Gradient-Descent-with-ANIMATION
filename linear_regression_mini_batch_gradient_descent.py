...
def mini_batch_gradient_descent(X,y,alpha,B1,B0,batch_size):
    rand_num = np.random.randint(X.shape[0],size= batch_size)
    X = X[rand_num]
    y = y[rand_num]
    y_pred = X*B1 + B0
    y_diff = (y - y_pred)
    t_B1 = -2*(np.sum(y_diff*X)/batch_size)
    t_B0 = -2*(np.sum(y_diff)/batch_size)
    B1 = B1 - alpha*t_B1
    B0 = B0 - alpha*t_B0
    return (B1, B0)
...
batch_size = 30  # not greater than the number of data points
